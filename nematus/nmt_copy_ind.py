...
def prepare_data_multi_src(seqs_x1, seqs_x2, seqs_y, weights=None, maxlen=None, n_words_src=30000,
    ...
    if maxlen is not None:
        ...
        for l_x1, s_x1, l_x2, s_x2, l_y, s_y, w in zip(lengths_x1, seqs_x1, lengths_x2, seqs_x2, lengths_y, seqs_y, weights):
            if l_x1 < maxlen and l_y < maxlen and l_x2 < maxlen:
    ...
    for idx, [s_x1, s_x2, s_y] in enumerate(zip(seqs_x1, seqs_x2, seqs_y)):
        ...
        x2[:, :lengths_x2[idx], idx] = zip(*s_x2)
...
def init_params(options):
    ...
    params = get_layer_param('ff')(options, params, prefix='ff_copy_lstm',
                                nin=options['dim']*2, nout=1, bias=False,
                                ortho=False)
...
def build_encoder(tparams, options, dropout, x1_mask=None, x2_mask=None, sampling=False):

    x1 = tensor.tensor3('x1', dtype='int64')
    x2 = tensor.tensor3('x2', dtype='int64')
    # source text; factors 1; length 5; batch size 10
    x1.tag.test_value = (numpy.random.rand(1, 5, 10)*100).astype('int64')
    x2.tag.test_value = (numpy.random.rand(1, 5, 10)*100).astype('int64')
    ...
    if options['use_dropout']:
        ...
        emb1 *= source1_dropout
        emb2 *= source2_dropout

        if sampling:
            emb1r *= source1_dropout
            emb2r *= source2_dropout
        else:
            # we drop out the same words in both directions
            emb1r *= source1_dropout[::-1]
            emb2r *= source2_dropout[::-1]


    ## level 1
    proj1 = get_layer_constr(options['encoder'])(tparams, emb1, options, dropout,
                                                prefix='encoder1',
                                                mask=x1_mask,
                                                ...
                                                profile=profile)
    proj1r = get_layer_constr(options['encoder'])(tparams, emb1r, options, dropout,
                                                 prefix='encoder1_r',
                                                 mask=x1r_mask,
                                                 dropout_probability_below=options['dropout_embedding'],
                                                 dropout_probability_rec=options['dropout_hidden'],
                                                 ...
                                                 truncate_gradient=options['encoder_truncate_gradient'],
                                                 profile=profile)
    proj2 = get_layer_constr(options['encoder'])(tparams, emb2, options, dropout,
                                                prefix='encoder2',
                                                mask=x2_mask,
                                                dropout_probability_below=options['dropout_embedding'],
                                                dropout_probability_rec=options['dropout_hidden'],
                                                ...
                                                truncate_gradient=options['encoder_truncate_gradient'],
                                                profile=profile)
    proj2r = get_layer_constr(options['encoder'])(tparams, emb2r, options, dropout,
                                                 prefix='encoder2_r',
                                                 mask=x2r_mask,
                                                 dropout_probability_below=options['dropout_embedding'],
                                                 dropout_probability_rec=options['dropout_hidden'],
    ...
    ctx2 = concatenate([proj2[0], proj2r[0][::-1]], axis=proj2[0].ndim-1)

    ## forward encoder layers after bidirectional layers are concatenate
    return x1, x2, ctx1, ctx2


# RNN decoder (including embedding and feedforward layer before output)
def build_decoder(tparams, options, y, ctx1, ctx2, init_state_gen, init_state_copy, dropout, x1_mask=None, x2_mask=None, 
    ...
    proj_copy = get_layer_constr(options['decoder'])(tparams, emb, options, dropout,
                                            prefix='decoder_copy',
                                            mask=y_mask, context=ctx1,
                                            context_mask=x1_mask,
                                            pctx_=pctx1_, 
                                            #cov_=cov1_, 
                                            one_step=one_step,
                                            init_state=init_state_gen[0],
                                            recurrence_transition_depth=options['dec_base_recurrence_transition_depth'],
                                            dropout_probability_below=options['dropout_embedding'],
                                            ...
                                            #cov=options['cov'],
                                            #align=align1)
    proj_gen = get_layer_constr(options['decoder'])(tparams, emb, options, dropout,
                                            prefix='decoder_gen',
                                            mask=y_mask, context=ctx2,
                                            context_mask=x2_mask,
                                            pctx_=pctx2_, 
                                            #cov_=cov2_, 
                                            one_step=one_step,
                                            init_state=init_state_copy[0],
                                            recurrence_transition_depth=options['dec_base_recurrence_transition_depth'],
                                            dropout_probability_below=options['dropout_embedding'],
                                            dropout_probability_ctx=options['dropout_hidden'],
                                            dropout_probability_rec=options['dropout_hidden'],
    ...
    next_state_gen = proj_gen[0]
    next_state_copy = proj_copy[0]
    next_state = tensor.concatenate([next_state_copy, next_state_gen], axis=-1)
    # weighted averages of context, generated by attention module
    ctxs_gen = proj_gen[1]
    ctxs_copy = proj_copy[1]
    ctxs = tensor.concatenate([ctxs_copy, ctxs_gen], axis=-1)
    # weights (alignment matrix)
    opt_ret['dec_alphas1'] = proj_copy[2]
    opt_ret['dec_alphas2'] = proj_gen[2]
    alpha1 = proj_copy[2] # (timestep, batch_size, length x1) copy_score
    alpha2 = proj_gen[2]
    #cov1 = proj[4]
    #cov2 = proj[5]
    #copy_score = proj_copy[3]
    copy_score = alpha1

    # we return state of each layer
    if sampling:
        ret_state_gen = next_state_gen.reshape((1, next_state_gen.shape[0], next_state_gen.shape[1]))
        ret_state_copy = next_state_copy.reshape((1, next_state_copy.shape[0], next_state_copy.shape[1]))
    else:
        ret_state_gen = None
        ret_state_copy = None
    ...
    logit_lstm = get_layer_constr('ff')(tparams, next_state_gen, options, dropout,
    ...
    logit_ctx = get_layer_constr('ff')(tparams, ctxs_gen, options, dropout,
                                   dropout_probability=options['dropout_hidden'],
    ...
    #copy_c_lstm = get_layer_constr('ff')(tparams, next_state_copy, options, dropout,
    #                                dropout_probability=options['dropout_hidden'],
    #                                prefix='ff_copy_lstm', activ='linear', bias=False)
    ...
    if sampling:
        pgen_bias = tensor.tile(tparams['pgen_bias'], (ctxs.shape[0], 1) )
    else:
        pgen_bias = tensor.tile(tparams['pgen_bias'], (ctxs.shape[0], ctxs.shape[1], 1) )
    ...
    return logit, opt_ret, ret_state_gen, ret_state_copy, pgen, copy_score
...
def build_model(tparams, options):
    ...
    x1_mask = tensor.matrix('x_mask', dtype=floatX)
    x2_mask = tensor.matrix('x_mask', dtype=floatX)
    y = tensor.matrix('y', dtype='int64')
    y_mask = tensor.matrix('y_mask', dtype=floatX)
    # source text length 5; batch size 10
    x1_mask.tag.test_value = numpy.ones(shape=(5, 10)).astype(floatX)
    x2_mask.tag.test_value = numpy.ones(shape=(5, 10)).astype(floatX)
    ...
    logit, opt_ret, _, _, pgen, copy_score = build_decoder(tparams, options, y, ctx1, ctx2, 
        init_state, init_state, dropout, x1_mask=x1_mask, x2_mask=x2_mask, y_mask=y_mask, cov1_=cov1_, cov2_=cov2_, 
...
def build_sampler(tparams, options, use_noise, trng, return_alignment=False):
    ...
    init_state_gen = tensor.tensor3('init_state', dtype=floatX)
    init_state_copy = tensor.tensor3('init_state', dtype=floatX)
    ...
    logit, opt_ret, ret_state_gen, ret_state_copy, pgen, copy_score = build_decoder(tparams, options, y, ctx1, ctx2, init_state_gen, init_state_copy, dropout, 
    ...
    if options['align']:
        if options['cov']:
            inps = [y, ctx1, ctx2, x_to_y_vocab, init_state_gen, init_state_copy, cov1_, cov2_, align1, align2]
        else:
            inps = [y, ctx1, ctx2, x_to_y_vocab, init_state_gen, init_state_copy, cov1_, cov2_, align1]
    else:
        inps = [y, ctx1, ctx2, x_to_y_vocab, init_state_gen, init_state_copy, cov1_, cov2_]
    #outs = [next_probs, next_sample, ret_state_gen, ret_state_copy, cov1, cov2]
    outs = [next_probs, next_sample, ret_state_gen, ret_state_copy]
...
def gen_sample(f_init, f_next, x, x_to_y_vocab, model_options=[None], trng=None, k=1, maxlen=30,
    ...
    next_state_gen = [None]*num_models
    next_state_copy = [None]*num_models
    ...
    for i in xrange(num_models):
        ...
        next_state_gen[i] = numpy.tile( ret[0] , (live_k, 1, 1))
        next_state_copy[i] = numpy.tile( ret[0] , (live_k, 1, 1))
    ...
    for ii in xrange(maxlen):
        for i in xrange(num_models):
            ctx1 = numpy.tile(ctx01[i], [live_k, 1])
            ctx2 = numpy.tile(ctx02[i], [live_k, 1])

            # for theano function, go from (batch_size, layers, dim) to (layers, batch_size, dim)
            next_state_gen[i] = numpy.transpose(next_state_gen[i], (1,0,2))
            next_state_copy[i] = numpy.transpose(next_state_copy[i], (1,0,2))
            cov1[i] = numpy.transpose(cov1[i], (1, 0))
            cov2[i] = numpy.transpose(cov2[i], (1, 0))

            inps = [next_w, ctx1, ctx2, x_to_y_vocab, next_state_gen[i], next_state_copy[i], cov1[i], cov2[i]]
            ...
            next_p[i], next_w_tmp, next_state_gen[i], next_state_copy[i] = ret[0], ret[1], ret[2], ret[3]
            # dummy LM states
        ...
        if stochastic:
            ...
            if argmax:
                ...
                for ti in xrange(live_k):
                    hyp_gen_states.append([copy.copy(next_gen_state[i][ti]) for i in xrange(num_models)])
                    hyp_copy_states.append([copy.copy(next_copy_state[i][ti]) for i in xrange(num_models)])
                ...
                new_cov1=[]
                new_cov2=[]
                for hyp_sample, hyp_gen_state, hyp_copy_state, hyp_lm_state, hyp_score, hyp_word_prob, hyp_c1, hyp_c2 in zip(hyp_samples, hyp_gen_states, hyp_copy_states, hyp_lm_states ,hyp_scores, word_probs, hyp_cov1, hyp_cov2):
                    if hyp_sample[-1]  > 0:
                        new_hyp_samples.append(copy.copy(hyp_sample))
                        new_hyp_gen_states.append(copy.copy(hyp_gen_state))
                        new_hyp_copy_states.append(copy.copy(hyp_copy_state))
                ...
                hyp_gen_states=new_hyp_gen_states
                hyp_copy_states=new_hyp_copy_states
                ...
                next_state_gen = [numpy.array(state) for state in zip(*hyp_gen_states)]
                next_state_copy = [numpy.array(state) for state in zip(*hyp_copy_states)]
            ...
            for idx, [ti, wi] in enumerate(zip(trans_indices, word_indices)):
                ...
                new_hyp_gen_states.append([copy.copy(next_state_gen[i][ti]) for i in xrange(num_models)])
                new_hyp_copy_states.append([copy.copy(next_state_copy[i][ti]) for i in xrange(num_models)])
            ...
            for idx in xrange(len(new_hyp_samples)):
                ...
                if new_hyp_samples[idx][-1] == 0:
                    ...
                    hyp_gen_states.append(copy.copy(new_hyp_gen_states[idx]))
                    hyp_copy_states.append(copy.copy(new_hyp_copy_states[idx]))
    ...
    for training_progress.eidx in xrange(training_progress.eidx, max_epochs):
        ...
        for x, y, a1, a2 in train:
            ...
            if valid is not None and validFreq and numpy.mod(training_progress.uidx, validFreq) == 0:
                use_noise.set_value(0.)
                valid_errs, alignment1, alignment2 = pred_probs(f_log_probs, prepare_data_multi_src,
                                        model_options, valid)
...
if __name__ == '__main__':
    ...
    network.add_argument('--decoder', type=str, default='gru_cond',
                         choices=['gru_cond', 'lstm_cond', 'gru_double_cond'],
