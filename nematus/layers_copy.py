...
layers = {'ff': ('param_init_fflayer', 'fflayer'),
          ...
          'gru_copy': ('param_init_gru_copy', 'gru_copy_layer'),
...
def param_init_gru_copy(options, params, prefix='gru_cond',
    ...
    # copy prob
    pgen_ctx = norm_weight(dim_c * 2, 1)
    params[pp(prefix, 'pgen_ctx')] = pgen_ctx
    pgen_hidden = norm_weight(dim, 1)
    params[pp(prefix, 'pgen_hidden')] = pgen_hidden
    pgen_emb = norm_weight((dim_below, 1))
    params[pp(prefix, 'pgen_emb')] = pgen_emb
    pgen_bias = norm_weight((1))
    params[pp(prefix, 'pgen_bias')] = pgen_bias

    return params


def gru_copy_layer(tparams, state_below, options, dropout, y_in_x, prefix='gru',
    ...
    def _step_slice(m_, x_, xx_, h_, ctx_, alpha1_, alpha2_, pctx1_, pctx2_, cc1_, cc2_, rec_dropout, ctx1_dropout, ctx2_dropout):
        ...
        alpha1 = alpha1 / alpha1.sum(0, keepdims=True) # (length_x1, batch_size)
